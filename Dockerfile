...
RUN apt-get update && apt-get install -y lsb-release ca-certificates apt-transport-https curl gnupg dpkg locales nginx-extras git wget curl gnupg && rm -rf /var/lib/apt/lists/* && localedef -i zh_CN -c -f UTF-8 -A /usr/share/locale/locale.alias zh_CN.UTF-8
